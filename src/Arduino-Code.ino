...
void buzzerUpdate() {
  ...
  switch (beepPattern) {
    case BEEP_SHORT:
      if (beepStep == 0) { digitalWrite(BUZZER_PIN, HIGH); beepNextMs = now + 70; beepStep++; }
      else { digitalWrite(BUZZER_PIN, LOW); beepPattern = BEEP_NONE; }
      break;

    case BEEP_DOUBLE:
      if (beepStep == 0) { digitalWrite(BUZZER_PIN, HIGH); beepNextMs = now + 60; beepStep++; }
      else if (beepStep == 1) { digitalWrite(BUZZER_PIN, LOW);  beepNextMs = now + 70; beepStep++; }
      else if (beepStep == 2) { digitalWrite(BUZZER_PIN, HIGH); beepNextMs = now + 90; beepStep++; }
      else { digitalWrite(BUZZER_PIN, LOW); beepPattern = BEEP_NONE; }
      break;

    case BEEP_ERROR:
      if (beepStep == 0) { digitalWrite(BUZZER_PIN, HIGH); beepNextMs = now + 200; beepStep++; }
      else { digitalWrite(BUZZER_PIN, LOW); beepPattern = BEEP_NONE; }
      break;

    case BEEP_LOCK:
      if (beepStep == 0) { digitalWrite(BUZZER_PIN, HIGH); beepNextMs = now + 700; beepStep++; }
      else { digitalWrite(BUZZER_PIN, LOW); beepPattern = BEEP_NONE; }
      break;

    case BEEP_TICK:
      if (beepStep == 0) { digitalWrite(BUZZER_PIN, HIGH); beepNextMs = now + 40; beepStep++; }
      else { digitalWrite(BUZZER_PIN, LOW); beepPattern = BEEP_NONE; }
      break;
